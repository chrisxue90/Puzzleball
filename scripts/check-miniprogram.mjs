...
if (!fs.existsSync(miniRoot)) errors.push('缺少 miniprogram/ 小程序源码目录');
...
if (projectConfig?.miniprogramRoot !== 'miniprogram/') errors.push('project.config.json 的 miniprogramRoot 必须为 miniprogram/');

const appConfig = readJson(path.join(miniRoot, 'app.json'));
for (const filename of ['app.js', 'app.json', 'app.wxss', 'sitemap.json']) {
  if (!fs.existsSync(path.join(miniRoot, filename))) errors.push(`缺少 miniprogram/${filename}`);
}

for (const page of appConfig?.pages || []) {
  for (const extension of ['.js', '.json', '.wxml', '.wxss']) {
    const file = path.join(miniRoot, `${page}${extension}`);
    if (!fs.existsSync(file)) errors.push(`页面缺少文件：miniprogram/${page}${extension}`);
  }
}
...
console.log(`小程序静态检查通过：${files.length} 个文件，${packageBytes} 字节。`);
